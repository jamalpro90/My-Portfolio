...
export default function Navbar() {
  ...
  useEffect(() => {
    window.addEventListener('scroll', function () {
      ...
      navbar.current.classList.toggle('dark:bg-black-09', scrollY > navbarHeight);
    });
  }, [scrollY]);
  ...
  return (
    <div ref={navbar} className="navbar w-full fixed z-10 top-0">
      ...
      <div className="container mx-auto flex justify-between items-center py-2">
        ...
        {/* ham menu */}
        <ul className="ham-menu text-3xl font-semibold font-u-mono text-white flex cursor-pointer tracking-wide w-40" onClick={mobileMenu}>
          <li className="ml-10" onClick={() => handleTheme(defaultTheme())}>
            <i className={`fas ${themeS}`}></i>
          </li>
          <li className="ml-10">
